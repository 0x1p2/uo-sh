...
function skillgain.fencing {

 if [[ $player_skill_fencing_status == "ulocked" ]]; then
  player_skill_total=$[ $player_skill_fencing + 0 ]
  randomint=`randomgen`
...
function skillgaincontrol {
 if [[ $randomint -le $player_skill_chance ]]; then
  player_skill_fencing=$[ $player_skill_fencing + 1 ]
  echo -e " [\033[1;36m+\033[m]  \033[1;33mYou gained [\033[1;36m+1\033[m\033[1;33m] to \033[1;34mFencing\033[m \033[1;33m[\033[m\033[1;36m$player_skill_fencing\033[m\033[1;33m]\033[m"
  if [[ $player_skill_fencing -ge 120 ]]; then
   player_skill_fencing=120
   player_skill_fencing_status=locked
  fi
 fi
}
