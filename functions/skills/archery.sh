...
function skillgain.archery {

 if [[ $player_skill_archery_status == "ulocked" ]]; then
  player_skill_total=$[ $player_skill_archery + 0 ]
  randomint=`randomgen`
...
function skillgaincontrol {
 if [[ $randomint -le $player_skill_chance ]]; then
  player_skill_archery=$[ $player_skill_archery + 1 ]
  echo -e " [\033[1;36m+\033[m]  \033[1;33mYou gained [\033[1;36m+1\033[m\033[1;33m] to \033[1;34mArchery\033[m \033[1;33m[\033[m\033[1;36m$player_skill_archery\033[m\033[1;33m]\033[m"
  if [[ $player_skill_archery -ge 120 ]]; then
   player_skill_archery=120
   player_skill_archery_status=locked
  fi
 fi
}
